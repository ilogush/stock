...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const receiptsData = [
      ...
      // Пижама W006
      { product_article: "W006", color_name: "Зайчики на розовом", sizes: { "XS": 11, "S": 15, "M": 15, "L": 12, "XL": 11, "XXL": 15 } },
      { product_article: "W006", color_name: "Шарики на голубом", sizes: { "XS": 3, "S": 4, "M": 4, "L": 3, "XL": 3, "XXL": 4 } },
      { product_article: "W006", color_name: "Сиренивые сердечки", sizes: { "XS": 15, "S": 10, "M": 10, "L": 15, "XL": 15, "XXL": 10 } },
      { product_article: "W006", color_name: "Белые цветы на голубом", sizes: { "XS": 3, "S": 4, "M": 4, "L": 3, "XL": 3, "XXL": 3 } },
      { product_article: "W006", color_name: "Белые цветы на брусничном", sizes: {} },
    ...
    for (const receiptData of receiptsData) {
      try {
        ...
        const { data: productData, error: productError } = await supabaseAdmin
          ...
          .eq('colors.name', receiptData.color_name)
          .select(`
            id, 
            name, 
            article,
            colors!products_color_id_fkey (id, name)
          `)
          .single();
        ...
        for (const [size, quantity] of Object.entries(receiptData.sizes)) {
          if (quantity > 0) {
            const receiptToCreate = {
              ...
              color_id: productData.colors.id,
              size_code: size,
              qty: quantity,
    ...
    console.error('Ошибка API create-receipts:', error);
