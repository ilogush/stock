...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
      // 🚀 КЭШИРОВАНИЕ для складских данных
      ...
      // Находим последний добавленный артикул по последнему поступлению
      let lastArticle = null;
      let lastArticleDate = null;
      
      // Получаем последнее поступление
      ...
      if (receiptError) {
        console.error('Ошибка при получении последнего поступления:', receiptError);
        ...
        if (itemsError) {
          console.error('Ошибка при получении товаров последнего поступления:', itemsError);
        } else if (lastReceiptItems && lastReceiptItems.length > 0) {
          lastArticle = lastReceiptItems[0].product?.article;
        }
      }
      
      // Фильтруем только товары последнего артикула
      const lastArticleItems = (receiptItems || []).filter((row: any) => 
        row.product?.article === lastArticle
      );

      // Агрегируем по product_id, size_code, color_id (из products) только для последнего артикула
      ...
      // 2) Вычитаем реализации только для последнего артикула
      let realItems: any[] = [];
      if (Object.keys(itemMap).length > 0 && lastArticle) {
        // Получаем все товары последнего артикула
      ...
      return res.status(200).json({
        ...
        sizes: sortedSizes,
        lastArticle: lastArticle, // Информация о последнем артикуле
