              ...
              <input
                ...
                onFocus={() => {
                  ...
                  if (!articleQuery.trim()) {
                    const allArticles = [...new Set(stock.map(s => s.article))]
                      .map(article => {
                        const item = stock.find(s => s.article === article);
                        return {
                          product_id: item.product_id,
                ...
                <tbody className="divide-y divide-gray-200">
                  {items.map(item => (
                    <React.Fragment key={item.id}>
                      ...
                      {stock.filter(s => s.product_id === item.product_id && s.size_code === item.size_code && parseInt(s.color_id) === item.color_id).map(s => (
                      ...
                      {stock.filter(s => s.product_id === item.product_id && s.size_code === item.size_code && parseInt(s.color_id) === item.color_id).length === 0 && (
