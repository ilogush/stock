...
const NewRealizationPage = () => {
  ...
  const [articleSuggestions, setArticleSuggestions] = useState<{product_id:number; article:string; name:string}[]>([]);
  ...
  const searchArticles = (query: string) => {
    ...
    const suggestions = stock
      ...
      .filter((value, index, self) => 
        self.findIndex(v => v.product_id === value.product_id) === index
      )
      .slice(0, 10);
  ...
  const handleArticleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!article.trim()) {
      setCurrentItem(prev => ({ ...prev, product_id: 0, size_code: '', color_id: 0, qty: 0 }));
      return;
    }

    // Ищем товар по артикулу в складских остатках
    const stockItem = stock.find(s => (s as any).article === article);
  ...
  const selectArticle = (product: {product_id:number; article:string; name:string}) => {
    setArticleQuery(product.article);
    setCurrentItem(prev => ({ ...prev, product_id: product.product_id, size_code: '', color_id: 0, qty: 0 }));
    setArticleError('');
    setShowArticleSuggestions(false);
  };
  ...
  // Максимальное количество для выбранного товара, размера и цвета
  ...
  const addItem = () => {
    ...
    // Проверяем, что товар есть на складе (ищем по названию цвета)
    ...
    if (!stockItem) {
      showToast('Товар не найден на складе', 'error');
      return;
    }

    if (currentItem.qty > stockItem.qty) {
      showToast(`Максимальное количество: ${stockItem.qty}`, 'error');
      return;
    }
    ...
    const newItem: RealizationItem = {
      ...
      qty: currentItem.qty,
      product_name: (stockItem as any).name || '',
      article: (stockItem as any).article || ''
    };

    setItems(prev => [...prev, newItem]);
    setCurrentItem({ product_id: 0, size_code: '', color_id: 0, qty: 0 });
    setArticleQuery('');
  };
    ...
    <div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-medium text-gray-900">Добавить товар</h3>
              ...
              >
                ...
                {currentItem.product_id ? stock
                  ...
                  .sort((a, b) => {
                    ...
                    const aValue = getSizeValue(a);
                    const bValue = getSizeValue(b);
                ...
                <tbody className="divide-y divide-gray-200">
                  {items.map(item => (
                    <React.Fragment key={item.id}>
                      ...
                      {stock.filter(s => s.product_id === item.product_id && s.size_code === item.size_code && s.color_id === item.color_id).map(s => (
                      ...
                      {stock.filter(s => s.product_id === item.product_id && s.size_code === item.size_code && s.color_id === item.color_id).length === 0 && (
