                ...
                {products.map((p) => (
                  <tr key={p.id} className="table-row-hover">
                    <td className="table-cell">
                      <div
                        className={`product-image-container relative rounded overflow-hidden bg-gray-100 ${roleName==='storekeeper' ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}
                        onClick={() => {
                          if (roleName === 'storekeeper') return;
                          router.push(`/products/${p.id}`);
                        }}
                        role={roleName==='storekeeper' ? 'img' : 'button'}
                        aria-label={roleName==='storekeeper' ? 'Просмотр недоступен' : 'Открыть карточку товара'}
                      >
