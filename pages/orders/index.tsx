...
const OrdersPage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    fetchOrders(1, statusFilter, pagination.limit, urlSearchQuery || '');
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      fetchOrders(pagination.page, statusFilter, pagination.limit, searchQuery);
    }, 30000);
  ...
  const handlePageChange = (page: number) => {
    ...
    const query: any = { page: String(page), limit: String(pagination.limit) };
    if (statusFilter && statusFilter !== 'all') query.status = statusFilter;
    if (searchQuery && searchQuery.trim()) query.search = searchQuery.trim();
    router.replace({ pathname: router.pathname, query }, undefined, { shallow: true });
    fetchOrders(page, statusFilter, pagination.limit, searchQuery);
  };
  ...
  const handleStatusFilterChange = (status: string) => {
    ...
    if (searchQuery && searchQuery.trim()) query.search = searchQuery.trim();
    router.replace({ pathname: router.pathname, query }, undefined, { shallow: true });
    fetchOrders(1, status, pagination.limit, searchQuery);
  };
  ...
  const debouncedSearch = useDebouncedCallback((q: string) => {
    ...
    router.replace({ pathname: router.pathname, query }, undefined, { shallow: true });
    fetchOrders(1, statusFilter, pagination.limit, q);
  }, 500);
