...
import { cache, cacheKeys } from '../unified/cache';
...
class NotificationService {
  ...
  async createBulkNotification(
    ...
    userIds.forEach(userId => this.clearUserNotificationCache(userId));
  ...
  async getUserNotifications(
    ...
    const cacheKey = cacheKeys.userNotifications(userId, options.limit, options.offset, options.unreadOnly, options.category);
    ...
    // Сохраняем в кэш
    cache.set(cacheKey, result, { ttl: 60000, tags: ['notifications'] });
  ...
  async getUnreadCount(userId: number): Promise<number> {
    ...
    // Сохраняем в кэш
    cache.set(cacheKey, unreadCount, { ttl: 30000, tags: ['notifications'] });
  ...
  private clearUserNotificationCache(userId: number): void {
    cache.invalidateByPattern(`notifications:${userId}`);
    cache.invalidateByPattern(`notifications:unread:${userId}`);
  }
