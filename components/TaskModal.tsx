...
export default function TaskModal({ task, isOpen, onClose, onStatusChange }: TaskModalProps) {
  ...
  const handleStatusChange = async () => {
    ...
    // Логика перехода статусов: new -> viewed -> in_progress -> done
    let next = 'new';
    if (currentStatus === 'new') next = 'viewed';
    else if (currentStatus === 'viewed') next = 'in_progress';
    else if (currentStatus === 'in_progress') next = 'done';
    else next = 'done';
      ...
      <div className="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            {(() => {
              const statusMap: { [key: string]: { text: string, color: string } } = {
                ...
                done: { text: 'выполненно', color: 'bg-gray-200 text-gray-800' }
              };
            ...
            <label className="block font-medium mb-1">
              ...
              {currentStatus !== 'done' && (
                <span className="text-xs text-gray-500 ml-2">
                  {currentStatus === 'new' ? '(необязательно)' : '(обязательно)'}
                </span>
              )}
            </label>
            <textarea
          ...
          <div className="flex gap-2 pt-2">
                          {currentStatus !== 'done' && user?.id === task.assignee_id && (
                <button
                ...
                >
                  {isLoading ? 'Обновление...' : 
                    ...
                    currentStatus === 'viewed' ? 'В процессе' :
                    currentStatus === 'in_progress' ? 'Выполненно' : 'Обновить'}
                </button>
              )}
            <button
              onClick={onClose}
          ...
          {currentStatus !== 'done' && user?.id !== task.assignee_id && (
            <div className="text-xs text-gray-500 text-center">
              Только исполнитель может изменять статус задачи
            </div>
          )}
        </div>
      </div>
